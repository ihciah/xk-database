...
class SearchForm():
    ...
    def search(self):
        ...
        sbycode=Course.session.query(Course,func.count(Xk.stuid).label('sum')).outerjoin(Xk,Xk.code==Course.code).group_by(Course.code).filter(Course.code.like(self.scode+'%')).limit(400)
        sr=sbycode.all()
        if len(sr)==0 or (len(sr)>0 and sr[0].Course is None):
            sbymajor=Course.session.query(Course,func.count(Xk.stuid).label('sum')).outerjoin(Xk,Xk.code==Course.code).group_by(Course.code).filter(Course.major.like(self.scode+'%')).limit(400)
            sr=sbymajor.all()
            if len(sr)==0 or (len(sr)>0 and sr[0].Course is None):
                sbydesp=Course.session.query(Course,func.count(Xk.stuid).label('sum')).outerjoin(Xk,Xk.code==Course.code).group_by(Course.code).filter(Course.desp.like(self.scode+'%')).limit(400)
                sr=sbydesp.all()
        for i in sr:
